...
public:
    ...
    std::vector<Comparable> BinarySearchTree<Comparable>::serialize() const
    {
        ...
        function<void(BinaryNode *)> inOrder = [&](BinaryNode *node)
        {
            if (node == nullptr)
                return;
            inOrder(node->left);
    ...
    {
        function<BinaryNode *(int, int)> buildTree = [&](int left, int right) -> BinaryNode *
        {
            if (left > right)
                return nullptr;
            int mid = left + (right - left) / 2;
            BinaryNode *node = new BinaryNode{vec[mid], nullptr, nullptr};
            node->left = buildTree(left, mid - 1);
            node->right = buildTree(mid + 1, right);
            return node;
        };
        makeEmpty();
        root = buildTree(0, vec.size() - 1);
    }
    ...
    {
        int count = 0;
        function<void(BinaryNode *&)> removeInRange = [&](BinaryNode *&node)
        {
            if (node == nullptr)
                return;
            if (node->element < L)
            {
                removeInRange(node->right);
            }
            else if (node->element > R)
            {
                removeInRange(node->left);
            }
            else
            {
                count++;
